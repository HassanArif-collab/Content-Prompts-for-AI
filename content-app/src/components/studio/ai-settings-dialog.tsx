...
export function AiSettingsDialog({ open, onOpenChange }: {
  ...
  const selectedCls = 'border-foreground bg-accent'
  const unselectedCls = 'border-border hover:bg-muted/40'
        ...
        <div className="space-y-5">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className={`p-3 rounded-lg border text-left transition-colors ${settings.provider === 'zai' ? selectedCls : unselectedCls}`}
              >
              ...
              <button
                ...
                className={`p-3 rounded-lg border text-left transition-colors ${settings.provider === 'ollama' ? selectedCls : unselectedCls}`}
              >
                <div className="flex items-center gap-2 mb-1">
          ...
          {settings.provider === 'ollama' && (
            <div className="space-y-3 p-4 rounded-lg border border-border bg-muted/20">
              ...
              <div className="flex items-start gap-2 text-xs text-muted-foreground bg-muted/40 border border-border rounded p-2">
            ...
            <div className="flex items-center justify-between">
              <div>
                <div className="text-sm font-medium flex items-center gap-2">
                  <Globe className="w-4 h-4 text-muted-foreground" /> Browser automation
                </div>
              ...
              <button
                ...
                className={`relative w-11 h-6 rounded-full transition-colors ${settings.browsingEnabled ? 'bg-foreground' : 'bg-muted'}`}
                aria-label="Toggle browsing"
              >
                <span className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-background transition-transform ${settings.browsingEnabled ? 'translate-x-5' : ''}`} />
              </button>
            </div>
